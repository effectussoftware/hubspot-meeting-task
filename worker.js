...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    offsetObject.after = parseInt(searchResult.paging?.next?.after);
    const meetingIds = data.map(meeting => ({id: meeting.id}));
    ...
    const contactAssociationsResults = (await (await hubspotClient.apiRequest({
      ...
      body: { inputs: meetingIds }
    })).json())?.results || [];

    // Get associated contacts' details
    const contactIds = contactAssociationsResults.reduce((ids, assoc) => {
      const id = assoc.to?.[0]?.id;
      if(id) ids.push({id})
      return ids;
    }, []);
    ...
    const contactEmails = Object.fromEntries(
      contactsResult.results.map(contact => [contact.id, contact.properties.email])
    );

    // Map meetings to contacts
    const meetingToContactMap = Object.fromEntries(
      contactAssociationsResults.map(assoc => {
        const contactId = assoc.to?.[0]?.id;
        return [assoc.from.id, contactEmails[contactId]];
      })
    );
    ...
    data.forEach(meeting => {
      ...
      const contactEmail = meetingToContactMap[meeting.id];
